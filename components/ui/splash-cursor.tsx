...
interface Pointer {
  ...
  down: boolean;
  moved: boolean;
  color: [number, number, number] | { r: number; g: number; b: number };
}

// Config interface removed as it was not being used
// If needed for future configuration, uncomment and use:
// interface Config {
...
function SplashCursor({
  ...
  useEffect(() => {
    ...
    function getWebGLContext(canvas: HTMLCanvasElement): WebGLContext {
      ...
      const isWebGL2 = !!gl;
      if (!isWebGL2) gl = (canvas.getContext('webgl', params) || canvas.getContext('experimental-webgl', params)) as WebGLRenderingContext | WebGL2RenderingContext | null;
      if (!gl) return;
    ...
    let dye: DoubleFrameBuffer, velocity: DoubleFrameBuffer, divergence: FrameBuffer, curl: FrameBuffer, pressure: DoubleFrameBuffer;
    ...
    function resizeFBO(target: FrameBuffer, w: number, h: number, internalFormat: number, format: number, type: number, param: number): FrameBuffer {
    ...
    function resizeDoubleFBO(target: DoubleFrameBuffer, w: number, h: number, internalFormat: number, format: number, type: number, param: number): DoubleFrameBuffer {
    ...
    function splatPointer(pointer: Pointer) {
      ...
      const color = Array.isArray(pointer.color)
    ...
    function getResolution(resolution: number): { width: number; height: number } {
      ...
      const finalAspectRatio = aspectRatio < 1 ? 1.0 / aspectRatio : aspectRatio;
      const min = Math.round(resolution);
      const max = Math.round(resolution * finalAspectRatio);
